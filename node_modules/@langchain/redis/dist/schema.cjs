...
* ```
*/
function buildMetadataSchema(metadataSchema, defaultSchema) {
	const updatedSchema = { ...defaultSchema };
	for (const fieldSchema of metadataSchema) switch (fieldSchema.type) {
		case "tag": {
			const tagOptions = {
				type: redis.SchemaFieldTypes.TAG,
				SEPARATOR: fieldSchema.options?.separator || DEFAULT_TAG_SEPARATOR
			};
			if (fieldSchema.options?.caseSensitive) tagOptions.CASESENSITIVE = true;
			if (fieldSchema.options?.noindex) tagOptions.NOINDEX = true;
			updatedSchema[fieldSchema.name] = tagOptions;
			break;
		}
		case "text": {
			const textOptions = { type: redis.SchemaFieldTypes.TEXT };
			if (fieldSchema.options?.weight !== void 0) textOptions.WEIGHT = fieldSchema.options.weight;
			if (fieldSchema.options?.noStem) textOptions.NOSTEM = true;
			if (fieldSchema.options?.noindex) textOptions.NOINDEX = true;
			if (fieldSchema.options?.sortable) textOptions.SORTABLE = true;
			updatedSchema[fieldSchema.name] = textOptions;
			break;
		}
		case "numeric": {
			const numericOptions = { type: redis.SchemaFieldTypes.NUMERIC };
			if (fieldSchema.options?.sortable) numericOptions.SORTABLE = true;
			if (fieldSchema.options?.noindex) numericOptions.NOINDEX = true;
			updatedSchema[fieldSchema.name] = numericOptions;
			break;
		}
		case "geo": {
			const geoOptions = { type: redis.SchemaFieldTypes.GEO };
			if (fieldSchema.options?.noindex) geoOptions.NOINDEX = true;
			updatedSchema[fieldSchema.name] = geoOptions;
			break;
		}
		default: updatedSchema[fieldSchema.name] = { type: redis.SchemaFieldTypes.TEXT };
	}
...
* ```
*/
function serializeMetadataField(fieldSchema, fieldValue) {
	switch (fieldSchema.type) {
		case "tag": return Array.isArray(fieldValue) ? fieldValue.join(fieldSchema.options?.separator || DEFAULT_TAG_SEPARATOR) : String(fieldValue);
		case "text": return String(fieldValue);
		case "numeric":
			if (fieldValue && typeof fieldValue === "object" && "getTime" in fieldValue && typeof fieldValue.getTime === "function") return Math.floor(fieldValue.getTime() / 1e3);
			return Number(fieldValue);
		case "geo":
			if (Array.isArray(fieldValue) && fieldValue.length === 2) return `${fieldValue[0]},${fieldValue[1]}`;
			return String(fieldValue);
		default: return String(fieldValue);
	}
}
/**
...
* ```
*/
function deserializeMetadataField(fieldSchema, fieldValue) {
	if (fieldValue === void 0 || fieldValue === null) return fieldValue;
	switch (fieldSchema.type) {
		case "tag": {
			const separator = fieldSchema.options?.separator || DEFAULT_TAG_SEPARATOR;
			if (typeof fieldValue === "string" && fieldValue.includes(separator)) return fieldValue.split(separator);
			return fieldValue;
		}
		case "numeric": return Number(fieldValue);
		case "geo":
...
* ```
*/
function inferMetadataSchema(documents) {
	if (!documents || documents.length === 0) return [];
	const fieldValues = /* @__PURE__ */ new Map();
	for (const doc of documents) {
		if (!doc.metadata || typeof doc.metadata !== "object") continue;
...
* ```
*/
function checkForSchemaMismatch(customSchema, inferredSchema) {
	if (customSchema.length !== inferredSchema.length) return true;
	const inferredMap = /* @__PURE__ */ new Map();
	for (const field of inferredSchema) inferredMap.set(field.name, field);
	for (const customField of customSchema) {
		const inferredField = inferredMap.get(customField.name);
		if (!inferredField) return true;
		if (customField.type !== inferredField.type) return true;
	}
	return false;
}
/**
...
*/
function isGeoCoordinate(value) {
...
function isNumberOrDate(value) {
	return typeof value === "number" || typeof value === "object" && value !== null && "getTime" in value && typeof value.getTime === "function";
}
/**
...
* ```
*/
function convertLegacySchema(metadataKey, legacySchema) {
	console.warn("DEPRECATION WARNING: The Record<string, CustomSchemaField> format for customSchema is deprecated. Please migrate to the new MetadataFieldSchema[] format. See https://js.langchain.com/docs/integrations/vectorstores/redis for migration guide. This legacy format will be removed in the next major version.");
	...
	for (const [fieldName, fieldConfig] of Object.entries(legacySchema)) {
		...
		switch (fieldConfig.type) {
			...
			case redis.SchemaFieldTypes.NUMERIC:
				type = "numeric";
				break;
			case redis.SchemaFieldTypes.GEO:
				type = "geo";
				break;
			default:
				type = "text";
				console.warn(`Unknown field type ${fieldConfig.type} for field ${fieldName}, defaulting to "text"`);
		}
		const options = {};
		if (fieldConfig.SEPARATOR !== void 0) options.separator = fieldConfig.SEPARATOR;
		if (fieldConfig.CASESENSITIVE !== void 0) options.caseSensitive = fieldConfig.CASESENSITIVE;
		if (fieldConfig.WEIGHT !== void 0) options.weight = fieldConfig.WEIGHT;
		if (fieldConfig.NOSTEM !== void 0) options.noStem = fieldConfig.NOSTEM;
		if (fieldConfig.SORTABLE !== void 0) options.sortable = fieldConfig.SORTABLE === true || fieldConfig.SORTABLE === "UNF";
		if (fieldConfig.NOINDEX !== void 0) options.noindex = fieldConfig.NOINDEX;
...
//#endregion
exports.DEFAULT_TAG_SEPARATOR = DEFAULT_TAG_SEPARATOR;
exports.buildMetadataSchema = buildMetadataSchema;
exports.checkForSchemaMismatch = checkForSchemaMismatch;
exports.convertLegacySchema = convertLegacySchema;
exports.deserializeMetadataField = deserializeMetadataField;
exports.inferMetadataSchema = inferMetadataSchema;
exports.serializeMetadataField = serializeMetadataField;
