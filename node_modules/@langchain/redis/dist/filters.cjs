...
* ```
*/
var AndFilter = class extends FilterExpression {
	filterType = "and";
	constructor(filters) {
		super();
...
* ```
*/
var OrFilter = class extends FilterExpression {
	filterType = "or";
	constructor(filters) {
		super();
		this.filters = filters;
...
* ```
*/
var TagFilter = class TagFilter extends FilterExpression {
	filterType = "tag";
	constructor(field, values, negate = false) {
		super();
		this.field = field;
		this.values = values;
		this.negate = negate;
	...
	*/
	static create(field) {
		return {
			eq: (values) => new TagFilter(field, values, false),
			ne: (values) => new TagFilter(field, values, true)
		};
	}
	toString() {
		if (!this.values || Array.isArray(this.values) && this.values.length === 0 || this.values && typeof this.values === "object" && "size" in this.values && this.values.size === 0) return "*";
		let valueStr;
		if (typeof this.values === "string") valueStr = this.values;
		else if (Array.isArray(this.values)) valueStr = this.values.join("|");
		else valueStr = Array.from(this.values).join("|");
...
* ```
*/
var NumericFilter = class NumericFilter extends FilterExpression {
	filterType = "numeric";
	...
	*/
	static create(field) {
		return {
			eq: (value) => new NumericFilter(field, "eq", value),
			ne: (value) => new NumericFilter(field, "ne", value),
			gt: (value) => new NumericFilter(field, "gt", value),
			gte: (value) => new NumericFilter(field, "gte", value),
			lt: (value) => new NumericFilter(field, "lt", value),
			lte: (value) => new NumericFilter(field, "lte", value),
			between: (min, max) => new NumericFilter(field, "between", [min, max])
		};
	}
	toString() {
		let rangeStr;
		switch (this.operator) {
			case "eq":
				...
				break;
			case "ne": return `(-@${this.field}:[${this.value} ${this.value}])`;
			case "gt":
			...
			case "lte":
				...
				break;
			case "between":
				if (Array.isArray(this.value)) rangeStr = `[${this.value[0]} ${this.value[1]}]`;
				else throw new Error("Between operator requires array of two numbers");
				break;
			default: throw new Error(`Unknown numeric operator: ${this.operator}`);
		}
...
* ```
*/
var TextFilter = class TextFilter extends FilterExpression {
	filterType = "text";
	constructor(field, query, operator = "exact", negate = false) {
		super();
		this.field = field;
		this.query = query;
		this.operator = operator;
	...
	*/
	static create(field) {
		return {
			eq: (query) => new TextFilter(field, query, "exact"),
			ne: (query) => new TextFilter(field, query, "exact", true),
			match: (query) => new TextFilter(field, query, "match"),
			wildcard: (query) => new TextFilter(field, query, "wildcard"),
			fuzzy: (query) => new TextFilter(field, query, "fuzzy")
		};
	}
	toString() {
...
* ```
*/
var GeoFilter = class GeoFilter extends FilterExpression {
	filterType = "geo";
	constructor(field, longitude, latitude, radius, unit = "km", negate = false) {
		super();
		this.field = field;
	...
	*/
	static create(field) {
		return {
			within: (longitude, latitude, radius, unit = "km") => new GeoFilter(field, longitude, latitude, radius, unit),
			outside: (longitude, latitude, radius, unit = "km") => new GeoFilter(field, longitude, latitude, radius, unit, true)
		};
	}
	toString() {
...
* ```
*/
var CustomFilter = class extends FilterExpression {
	filterType = "custom";
	constructor(query) {
		super();
		this.query = query;
...
* ```
*/
var TimestampFilter = class TimestampFilter extends FilterExpression {
	...
	*/
	static create(field) {
		return {
			eq: (value) => new TimestampFilter(field, "eq", value),
			ne: (value) => new TimestampFilter(field, "ne", value),
			gt: (value) => new TimestampFilter(field, "gt", value),
			gte: (value) => new TimestampFilter(field, "gte", value),
			lt: (value) => new TimestampFilter(field, "lt", value),
			lte: (value) => new TimestampFilter(field, "lte", value),
			between: (start, end) => new TimestampFilter(field, "between", [start, end])
		};
	}
	/**
	...
	*/
	toEpoch(value) {
		return typeof value === "object" && value && "getTime" in value ? Math.floor(value.getTime() / 1e3) : value;
	}
	toString() {
		let rangeStr;
		switch (this.operator) {
			case "eq": {
				const eqValue = this.toEpoch(this.value);
				rangeStr = `[${eqValue} ${eqValue}]`;
				break;
			}
			case "ne": {
				const neValue = this.toEpoch(this.value);
				return `(-@${this.field}:[${neValue} ${neValue}])`;
			}
			case "gt": {
				const gtValue = this.toEpoch(this.value);
				rangeStr = `[(${gtValue} +inf]`;
				break;
			}
			case "gte": {
				const gteValue = this.toEpoch(this.value);
				rangeStr = `[${gteValue} +inf]`;
				break;
			}
			case "lt": {
				const ltValue = this.toEpoch(this.value);
				rangeStr = `[-inf (${ltValue}]`;
				break;
			}
			case "lte": {
				const lteValue = this.toEpoch(this.value);
			...
			case "between":
				...
				} else throw new Error("Between operator requires array of two values");
				break;
			default: throw new Error(`Unknown timestamp operator: ${this.operator}`);
		}
		const filter = `@${this.field}:${rangeStr}`;
		return this.negate ? `(-${filter})` : filter;
	}
