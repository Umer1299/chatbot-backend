import { AndFilter, Custom, CustomFilter, FilterExpression, Geo, GeoFilter, Num, NumericFilter, OrFilter, Tag, TagFilter, Text, TextFilter, Timestamp, TimestampFilter } from "./filters.js";
import { buildMetadataSchema, checkForSchemaMismatch, deserializeMetadataField, inferMetadataSchema, serializeMetadataField } from "./schema.js";
...
* ```
*/
var FluentRedisVectorStore = class FluentRedisVectorStore extends VectorStore {
	redisClient;
	indexName;
	indexOptions;
	createIndexOptions;
	...
	*/
	async addVectors(vectors, documents, { keys, batchSize = 1e3 } = {}) {
		...
		await Promise.all(vectors.map(async (vector, idx) => {
			const key = keys && keys.length ? keys[idx] : `${this.keyPrefix}${v4()}`;
			...
			if (this.customSchema && this.customSchema.length > 0) for (const fieldSchema of this.customSchema) {
				...
				if (fieldValue !== void 0 && fieldValue !== null) hashFields[fieldSchema.name] = serializeMetadataField(fieldSchema, fieldValue);
			}
			...
			for (const res of results.documents) if (res.value) {
				...
				if (document.vector_score) {
					...
					if (this.customSchema && this.customSchema.length > 0) for (const fieldSchema of this.customSchema) {
						...
						if (fieldValue !== void 0 && fieldValue !== null) metadata[fieldSchema.name] = deserializeMetadataField(fieldSchema, fieldValue);
					}
	...
	*/
	async createIndex(documents, dimensions = 1536) {
		...
		if (documents && documents.length > 0) {
			...
			if (checkForSchemaMismatch(this.customSchema, inferredSchema)) console.warn("The custom schema does not match the metadata schema inferred from the documents. This is not necessarily an issue, but could indicate an invalid custom schema.");
		}
		let schema = {
			[this.vectorKey]: {
...
//#endregion
export { FluentRedisVectorStore, RedisVectorStoreAdvanced };
//# sourceMappingURL=vectorstores_fluent.js.map