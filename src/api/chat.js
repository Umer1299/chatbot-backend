...
router.post('/', tokenAuth, domainRestriction, async (req, res) => {
  ...
  const processResponse = async (fullResponse, usedModel) => {
    ...
    tasks.push((async () => { try { const inputTokens = Math.ceil(fullSystemPrompt.length / 4); const outputTokens = Math.ceil(fullResponse.length / 4); const cost = (inputTokens * 0.000003) + (outputTokens * 0.000015); await redisClient.incrbyfloat(`cost_usd:${req.namespace}`, cost); } catch {} })());
